...
// TODO: Change this if the production domain changes.
const BASE_URL = 'https://www.sna-photo.com';
const ROOT = path.join(__dirname, '..');
const BLOG_DIR = path.join(ROOT, 'blog');
const REQUIRED = ['title', 'date', 'isoDate', 'excerpt', 'slug', 'tags', 'content', 'sources'];
...
function validatePost(post) {
  for (const key of REQUIRED) if (!post[key]) throw new Error(`Generated post is missing ${key}`);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(post.isoDate)) throw new Error('isoDate must use YYYY-MM-DD');
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(post.slug)) throw new Error('slug must be lowercase and URL safe');
  ...
  if (!Array.isArray(post.sources) || post.sources.length < 2) throw new Error('Post must include at least two research sources');
  const domains = new Set();
  for (const source of post.sources) {
    if (!source.title || !source.publisher || !source.url) throw new Error('Each source needs title, publisher, and url');
    let url;
    try { url = new URL(source.url); } catch { throw new Error(`Invalid source URL: ${source.url}`); }
    if (url.protocol !== 'https:') throw new Error(`Source must use HTTPS: ${source.url}`);
    domains.add(url.hostname.replace(/^www\./, ''));
  }
  if (domains.size < 2) throw new Error('Research must use at least two independent domains');
  if (/<script|<style|\son\w+=/i.test(post.content)) throw new Error('Unsafe HTML found in generated content');
  const robotic = ['in conclusion', 'delve', 'tapestry', 'it is important to note', 'in today\'s fast-paced world'];
  for (const phrase of robotic) if (post.content.toLowerCase().includes(phrase)) throw new Error(`Robotic phrase found: ${phrase}`);
  return post;
}

function readPosts() {
  if (!fs.existsSync(BLOG_DIR)) return [];
  return fs.readdirSync(BLOG_DIR).filter(file => file.endsWith('.html')).map(file => {
    const html = fs.readFileSync(path.join(BLOG_DIR, file), 'utf8');
    const match = html.match(/<!-- METADATA: (.*?) -->/);
    if (!match) return null;
    try { return JSON.parse(match[1]); } catch { return null; }
  }).filter(Boolean).sort((a, b) => b.isoDate.localeCompare(a.isoDate));
}

function syncIndex() {
  const posts = readPosts();
  const cards = posts.map(post => `<article class="blog-card">
  <a class="journal-card-image" href="blog/${escapeHtml(post.slug)}.html" aria-label="Read ${escapeHtml(post.title)}"><img src="assets/images/new/680818044_122107018856735434_4454822386258834117_n.webp" alt="" loading="lazy"></a>
  ...
  const end = '<!-- POSTS_END -->';
  if (!index.includes(start) || !index.includes(end)) throw new Error('POSTS markers are missing from blog.html');
  fs.writeFileSync(indexPath, index.replace(new RegExp(`${start}[\\s\\S]*?${end}`), `${start}\n${cards}\n${end}`));
...
function generateSitemap(posts) {
  ...
  const urls = pages.map((url, i) => `  <url><loc>${BASE_URL}${url || '/'}</loc><lastmod>${today}</lastmod><priority>${i === 0 ? '1.0' : '0.8'}</priority></url>`);
  posts.forEach(post => urls.push(`  <url><loc>${BASE_URL}/blog/${escapeHtml(post.slug)}.html</loc><lastmod>${post.isoDate}</lastmod><priority>0.7</priority></url>`));
  fs.writeFileSync(path.join(ROOT, 'sitemap.xml'), `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.join('\n')}\n</urlset>\n`);
}

async function generateBlog() {
  if (!process.env.OPENAI_API_KEY) throw new Error('Set OPENAI_API_KEY before generating a post');
  const OpenAI = require('openai');
  const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
  const topics = ['best places for dog photos near Marietta GA', 'prepare a dog for a photo session', 'prepare a cat for portraits', 'senior pet photography', 'outdoor versus studio pet portraits', 'what to bring to a pet photography session', 'seasonal pet photos in Georgia', 'nervous pets during photo sessions'];
  const existing = readPosts().map(post => post.title).join('; ') || 'None yet';
...
Return only valid JSON with title, date, isoDate, excerpt, slug, tags (array), content (semantic HTML using h2, h3, p, ul, li, and inline a tags only), and sources. Sources must be an array of objects with title, publisher, and the direct HTTPS url. Cite factual claims naturally with links in the article. End with a warm invitation to book, but do not invent an offer or urgency.`,
  ...
  const metadata = JSON.stringify({ title: post.title, date: post.date, isoDate: post.isoDate, excerpt: post.excerpt, slug: post.slug, tags: post.tags, sources: post.sources }).replace(/-->/g, '--&gt;');
  ...
  const html = template.replaceAll('{{POST_TITLE}}', escapeHtml(post.title)).replaceAll('{{POST_DATE}}', escapeHtml(post.date)).replaceAll('{{POST_ISO_DATE}}', escapeHtml(post.isoDate)).replaceAll('{{POST_EXCERPT}}', escapeHtml(post.excerpt)).replaceAll('{{POST_SLUG}}', escapeHtml(post.slug)).replace('{{POST_CONTENT}}', `${post.content}${sourcesHtml}`).replace('{{POST_METADATA}}', metadata);
  const destination = path.join(BLOG_DIR, `${post.slug}.html`);
  if (fs.existsSync(destination)) throw new Error(`Post already exists: ${post.slug}`);
...
(async () => {
  try { process.argv.includes('--sync-only') ? syncIndex() : await generateBlog(); }
  catch (error) { console.error(`Blog generation failed: ${error.message}`); process.exitCode = 1; }
})();
